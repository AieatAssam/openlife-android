...
The proposed C0 access policy relies on the Android device lock and app sandbox; it has no separate biometric or passcode lock. This does not protect against someone using an already unlocked phone. An app-lock requirement must be settled before distributing C0 to real users, since changing authentication-bound key access affects the lifecycle design.
...
| C1 Text provenance | Local OCR with text linked to actual image regions; user can inspect and correct it | Semantic records and actions |
...
The proposed C0 defaults are two modules, minSdk 29, Room with maintained SQLCipher, bounded whole-file authenticated reads and device-lock-only access. Record any change before implementation. The separate app-lock decision must be resolved before real-user distribution. The open-source licence must be selected before publishing. An encrypted export, sync, family sharing or external action would change the strict no-egress contract and is not authorised by this roadmap.
